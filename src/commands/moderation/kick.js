...
module.exports = {
    ...
    callback: async (client, interaction) => {
        ...
        if (targetUser.id === guild.ownerId) {
            return await interaction.editReply('You can\'t kick that user because they\'re the server owner.');
        }
        ...
        if (targetUserRolePosition >= requestUserRolePosition) {
            return await interaction.editReply('You can\'t kick that user because they have the same/higher role than you.');
        }

        if (targetUserRolePosition >= botRolePosition) {
            return await interaction.editReply('I can\'t kick that user because they have the same/higher role than me.');
        }

        try {
            await targetUser.kick({ reason });
            return await interaction.editReply(`User ${targetUser} was kicked\nReason: ${reason}`);
        } catch (error) {
